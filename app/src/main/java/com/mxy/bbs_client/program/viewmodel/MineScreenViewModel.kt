...
class MineScreenViewModel: ViewModel() {
    private val _mineScreenState = MutableStateFlow(MineScreenState(false, null))
    val mineScreenState = _mineScreenState.asStateFlow()
    fun loginSuccessfully(username: String) {
        _mineScreenState.value = MineScreenState(true, username)
    }
    fun logout() {
        _mineScreenState.value = MineScreenState(false, null)
    }
}